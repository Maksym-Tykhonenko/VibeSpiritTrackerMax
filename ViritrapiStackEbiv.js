...
import { SafeAreaProvider as VylzenCushion } from 'react-native-safe-area-context';
import { NavigationContainer as OrbitNavCradle } from '@react-navigation/native';
import { GestureHandlerRootView as GripRuneVault } from 'react-native-gesture-handler';
import { createNativeStackNavigator as weaveStackSpire } from '@react-navigation/native-stack';
import ProductScreen from './VibeSpiritTrackerSrc/VibeSpiritTrackerScreens/ProductScreen';
import StormIgniteLoader from './VibeSpiritTrackerSrc/VibeSpiritTrackerScreens/VibeSpiritTrackerLoading';
import SkyrVibeCoreDeck from './VibeSpiritTrackerSrc/VibeSpiritTrackerScreens/RtiripsObruchScn';
import SpiritWalkIntro from './VibeSpiritTrackerSrc/VibeSpiritTrackerScreens/VibeSpiritTrackerOnboarding';

const SpireStack = weaveStackSpire();
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';

const VibeRouteNebula = () => {
  const NebulaScreen = SpireStack.Screen;
  const NebulaFlow = SpireStack.Navigator;
  ...
  const INITIAL_URL = `https://bright-stream-plus.fun/`;
  const URL_IDENTIFAIRE = `edgz2Pu6`;
  ...
  OneSignal.initialize('ad0e7644-b224-4cd8-a935-5aea1b6802d6');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2026-01-29T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
    ...
    if (isFatch) {
      return (
        <NebulaFlow screenOptions={{ headerShown: false }}>
          <NebulaScreen
            initialParams={{
              responseToPushPermition,
              product: finalLink,
    ...
    return (
      <NebulaFlow
        initialRouteName="VibeSpiritTrackerOnboarding"
        screenOptions={{
      ...
      >
        <NebulaScreen
          name="VibeSpiritTrackerLoading"
          component={StormIgniteLoader}
          options={{
            ...transitGlyphs.driftFade,
            title: 'Storm Loading',
          }}
        />

        <NebulaScreen
          name="RtiripsObruchScn"
        ...
        <NebulaScreen
          name="VibeSpiritTrackerOnboarding"
          component={SpiritWalkIntro}
          options={{
            ...transitGlyphs.softBlink,
            title: 'Onboard Tubes',
          }}
        />
      </NebulaFlow>
  ...
  return (
    <OrbitNavCradle>
      {!isLoading ? <StormIgniteLoader /> : <Route isFatch={route} />}
    </OrbitNavCradle>
  );
};

const SpiritRootHarbor = () => {
  return (
    <GripRuneVault style={{ flex: 1, backgroundColor: '#0D0D0E' }}>
      <VylzenCushion>
        <VibeRouteNebula />
      </VylzenCushion>
    </GripRuneVault>
...
export default SpiritRootHarbor;
